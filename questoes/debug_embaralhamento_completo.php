<?php
session_start();
require_once __DIR__ . '/conexao.php';

echo "<h1>DEBUG EMBARALHAMENTO COMPLETO</h1>";

// Gerar seed de sessão para embaralhamento consistente
if (!isset($_SESSION['quiz_seed'])) {
    $_SESSION['quiz_seed'] = rand(1, 10000);
}

echo "<h2>Seed de sessão: " . $_SESSION['quiz_seed'] . "</h2>";

// Testar questão 92
$questao_id = 92;

try {
    // Buscar alternativas
    $stmt_alt = $pdo->prepare("SELECT * FROM alternativas WHERE id_questao = ? ORDER BY id_alternativa");
    $stmt_alt->execute([$questao_id]);
    $alternativas = $stmt_alt->fetchAll(PDO::FETCH_ASSOC);
    
    echo "<h3>Questão #$questao_id:</h3>";
    
    // Mostrar alternativas originais
    echo "<h4>1. Alternativas ORIGINAIS (ordem do banco):</h4>";
    $letras = ['A', 'B', 'C', 'D', 'E'];
    foreach ($alternativas as $index => $alt) {
        $letra = $letras[$index] ?? ($index + 1);
        $correta = $alt['eh_correta'] ? ' (CORRETA)' : '';
        echo "<p>$letra) " . htmlspecialchars($alt['texto']) . $correta . " [ID: " . $alt['id_alternativa'] . "]</p>";
    }
    
    // Testar diferentes métodos de embaralhamento
    echo "<h4>2. Testando diferentes métodos de embaralhamento:</h4>";
    
    // Método 1: Seed baseado em sessão (como no quiz_vertical_filtros.php)
    echo "<h5>Método 1: Seed baseado em sessão</h5>";
    $seed1 = $questao_id * 1000 + $_SESSION['quiz_seed'];
    srand($seed1);
    $alt1 = $alternativas;
    shuffle($alt1);
    echo "<p>Seed: $seed1</p>";
    foreach ($alt1 as $index => $alt) {
        $letra = $letras[$index] ?? ($index + 1);
        $correta = $alt['eh_correta'] ? ' (CORRETA)' : '';
        echo "<p>$letra) " . htmlspecialchars($alt['texto']) . $correta . " [ID: " . $alt['id_alternativa'] . "]</p>";
    }
    
    // Método 2: Seed aleatório (como no quiz.php)
    echo "<h5>Método 2: Seed aleatório</h5>";
    $seed2 = $questao_id + time() + rand(1, 1000);
    srand($seed2);
    $alt2 = $alternativas;
    shuffle($alt2);
    echo "<p>Seed: $seed2</p>";
    foreach ($alt2 as $index => $alt) {
        $letra = $letras[$index] ?? ($index + 1);
        $correta = $alt['eh_correta'] ? ' (CORRETA)' : '';
        echo "<p>$letra) " . htmlspecialchars($alt['texto']) . $correta . " [ID: " . $alt['id_alternativa'] . "]</p>";
    }
    
    // Método 3: Seed fixo baseado apenas no ID
    echo "<h5>Método 3: Seed fixo baseado apenas no ID</h5>";
    $seed3 = $questao_id * 1000;
    srand($seed3);
    $alt3 = $alternativas;
    shuffle($alt3);
    echo "<p>Seed: $seed3</p>";
    foreach ($alt3 as $index => $alt) {
        $letra = $letras[$index] ?? ($index + 1);
        $correta = $alt['eh_correta'] ? ' (CORRETA)' : '';
        echo "<p>$letra) " . htmlspecialchars($alt['texto']) . $correta . " [ID: " . $alt['id_alternativa'] . "]</p>";
    }
    
    // Testar embaralhamento múltiplas vezes com o mesmo método
    echo "<h4>3. Testando embaralhamento múltiplas vezes (método 1):</h4>";
    
    for ($i = 1; $i <= 3; $i++) {
        echo "<h5>Teste $i:</h5>";
        
        // Usar seed baseado na sessão + número do teste
        $seed = $questao_id * 1000 + $_SESSION['quiz_seed'] + $i;
        srand($seed);
        
        $alt_emb = $alternativas;
        shuffle($alt_emb);
        
        echo "<p>Seed: $seed</p>";
        foreach ($alt_emb as $index => $alt) {
            $letra = $letras[$index] ?? ($index + 1);
            $correta = $alt['eh_correta'] ? ' (CORRETA)' : '';
            echo "<p>$letra) " . htmlspecialchars($alt['texto']) . $correta . " [ID: " . $alt['id_alternativa'] . "]</p>";
        }
        echo "<hr>";
    }
    
    // Verificar se o embaralhamento está funcionando
    echo "<h4>4. Verificação do embaralhamento:</h4>";
    
    // Fazer 5 embaralhamentos com seeds diferentes
    $embaralhamentos = [];
    for ($i = 0; $i < 5; $i++) {
        $seed = $questao_id * 1000 + $_SESSION['quiz_seed'] + $i;
        srand($seed);
        $alt_emb = $alternativas;
        shuffle($alt_emb);
        $embaralhamentos[] = $alt_emb;
    }
    
    // Verificar se os embaralhamentos são diferentes
    $sao_diferentes = true;
    $diferencas = 0;
    for ($i = 0; $i < count($embaralhamentos) - 1; $i++) {
        for ($j = $i + 1; $j < count($embaralhamentos); $j++) {
            $ids1 = array_column($embaralhamentos[$i], 'id_alternativa');
            $ids2 = array_column($embaralhamentos[$j], 'id_alternativa');
            if ($ids1 === $ids2) {
                $sao_diferentes = false;
            } else {
                $diferencas++;
            }
        }
    }
    
    echo "<p>Total de comparações: " . (count($embaralhamentos) * (count($embaralhamentos) - 1) / 2) . "</p>";
    echo "<p>Diferencas encontradas: $diferencas</p>";
    
    if ($sao_diferentes) {
        echo "<p style='color: green;'>✅ EMBARALHAMENTO FUNCIONANDO! As alternativas mudam de posição.</p>";
    } else {
        echo "<p style='color: red;'>❌ EMBARALHAMENTO NÃO FUNCIONANDO! As alternativas não mudam de posição.</p>";
    }
    
    // Testar se o problema está no PHP ou na lógica
    echo "<h4>5. Teste de embaralhamento básico do PHP:</h4>";
    
    $array_teste = [1, 2, 3, 4, 5];
    echo "<p>Array original: " . implode(', ', $array_teste) . "</p>";
    
    for ($i = 0; $i < 3; $i++) {
        $array_emb = $array_teste;
        shuffle($array_emb);
        echo "<p>Embaralhamento $i: " . implode(', ', $array_emb) . "</p>";
    }
    
} catch (Exception $e) {
    echo "<p style='color: red;'>❌ Erro: " . $e->getMessage() . "</p>";
}

echo "<h2>6. Próximos passos:</h2>";
echo "<p>1. Se o embaralhamento básico do PHP está funcionando, o problema pode estar na lógica</p>";
echo "<p>2. Se não está funcionando, pode ser um problema do servidor</p>";
echo "<p>3. Verificar se há conflito entre diferentes arquivos</p>";
?>
