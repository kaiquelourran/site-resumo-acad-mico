...
echo "<h1>TESTE DE CLIQUE REAL NO QUIZ</h1>";
...
echo "<h2>Simulando clique na alternativa C da questão 99:</h2>";
...
try {
    ...
    if ($alternativa_correta && $id_alternativa) {
        ...
        foreach ($alternativas_questao as $index => $alt) {
            if ($alt['id_alternativa'] == $alternativa_correta['id_alternativa']) {
                $letra_correta_json = $letras[$index] ?? ($index + 1);
                break;
            }
        }
        ...
        $resposta_json = [
            ...
            'alternativa_correta' => $letra_correta_json,
            'explicacao' => '',
